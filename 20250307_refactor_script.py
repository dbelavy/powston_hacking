...
# Section to pick up all the variables available from the environment
print("Environment Variables:")
for key, value in os.environ.items():
    print(f"{key}={value}")
print("-" * 30)

# Section to perform analyses and functions to understand the environment
print("Environment Analysis:")
print(f"Operating System: {os.name}")
print("-" * 30)

# Data Validation Section
# This section validates critical data points and provides default values where appropriate

# Helper function to validate and provide defaults for numeric values
def safe_float(value, default_value, name):
    """Safely convert a value to float, with a default if it fails."""
    try:
        return float(value)
    except (TypeError, ValueError):
        print(f"Warning: {name} is invalid or missing. Using default value: {default_value}")
        return default_value

# Helper function to validate and provide defaults for list values
def safe_list(value, default_value, name):
    """Safely validate a list, with a default if it fails."""
    if not isinstance(value, list):
        print(f"Warning: {name} is not a list. Using default.")
        return default_value
    ...
    try:
        ...
        print(f"Warning: {name} contains invalid elements. Using default.")
        return default_value

# Validate critical data points with safe defaults

# Current pricing data
try:
    buy_price = safe_float(buy_price, 30.0, "buy_price")
    if buy_price < 0:
        print(f"Warning: buy_price ({buy_price}) is negative. This is unusual but allowed.")
except NameError:
    print("Warning: buy_price is not defined. Using default value: 30.0")
    buy_price = 30.0

try:
    sell_price = safe_float(sell_price, 10.0, "sell_price")
    if sell_price < 0:
        print(f"Warning: sell_price ({sell_price}) is negative. This is unusual but allowed.")
except NameError:
    print("Warning: sell_price is not defined. Using default value: 10.0")
    sell_price = 10.0

try:
    rrp = safe_float(rrp, 50.0, "rrp")
except NameError:
    print("Warning: rrp is not defined. Using default value: 50.0")
    rrp = 50.0

# Forecast data
default_buy_forecast = [30.0, 31.0, 32.0, 33.0, 34.0, 35.0, 36.0, 37.0]
default_sell_forecast = [10.0, 9.5, 9.0, 8.5, 8.0, 7.5, 7.0, 6.5]

try:
    buy_forecast = safe_list(buy_forecast, default_buy_forecast, "buy_forecast")
    if not buy_forecast:  # Empty list
        print("Warning: buy_forecast is empty. Using default values.")
        buy_forecast = default_buy_forecast
except NameError:
    print("Warning: buy_forecast is not defined. Using default values.")
    buy_forecast = default_buy_forecast

try:
    sell_forecast = safe_list(sell_forecast, default_sell_forecast, "sell_forecast")
    if not sell_forecast:  # Empty list
        print("Warning: sell_forecast is empty. Using default values.")
        sell_forecast = default_sell_forecast
except NameError:
    print("Warning: sell_forecast is not defined. Using default values.")
    sell_forecast = default_sell_forecast

# Power and battery data
try:
    house_power = safe_float(house_power, 2000.0, "house_power")
    if house_power < 0:
        print(f"Warning: house_power ({house_power}) is negative. Setting to 0.")
        house_power = 0.0
except NameError:
    print("Warning: house_power is not defined. Using default value: 2000.0")
    house_power = 2000.0

try:
    solar_power = safe_float(solar_power, 0.0, "solar_power")
    if solar_power < 0:
        print(f"Warning: solar_power ({solar_power}) is negative. Setting to 0.")
        solar_power = 0.0
except NameError:
    print("Warning: solar_power is not defined. Using default value: 0.0")
    solar_power = 0.0

try:
    battery_soc = safe_float(battery_soc, 50.0, "battery_soc")
    if battery_soc < 0:
        print(f"Warning: battery_soc ({battery_soc}) is negative. Setting to 0.")
        battery_soc = 0.0
    elif battery_soc > 100:
        print(f"Warning: battery_soc ({battery_soc}) is above 100%. Setting to 100.")
        battery_soc = 100.0
except NameError:
    print("Warning: battery_soc is not defined. Using default value: 50.0")
    battery_soc = 50.0

try:
    battery_capacity = safe_float(battery_capacity, 25600.0, "battery_capacity")
    if battery_capacity <= 0:
        print(f"Warning: battery_capacity ({battery_capacity}) is not positive. Using default value: 25600.0")
        battery_capacity = 25600.0
except NameError:
    print("Warning: battery_capacity is not defined. Using default value: 25600.0")
    battery_capacity = 25600.0

# Validate num_inverters to prevent division by zero
try:
    num_inverters = int(num_inverters)
    if num_inverters <= 0:
        print(f"Warning: num_inverters ({num_inverters}) must be positive. Setting to 1.")
        num_inverters = 1
except (NameError, TypeError, ValueError):
    print("Warning: num_inverters is not defined or invalid. Using default value: 1")
    num_inverters = 1
...
min_house_power = [
    1000.0,  # 12:00 AM - 1:00 AM
    1000.0,  # 1:00 AM - 2:00 AM
    1000.0,  # 2:00 AM - 3:00 AM
    1000.0,  # 3:00 AM - 4:00 AM
    1000.0,  # 4:00 AM - 5:00 AM
    1500.0,  # 5:00 AM - 6:00 AM
    ...
    1000.0,  # 10:00 PM - 11:00 PM
    1000.0   # 11:00 PM - 12:00 AM
]
...
facility_name = "3 Qld"
num_inverters = 1  # Number of inverters at this facility
...
timezone = 10.0  # noqa Local timezone +/- UTC
...
# Calculate total expected consumption until lowest buy price
def calculate_expected_consumption():
    total_consumption = 0
    current_hour = local_time.hour
    hours_to_check = int(hours_until_lowest_buy)

    # Calculate scaling factor based on current usage vs typical usage
    typical_current_hour_power = min_house_power[current_hour] / num_inverters
    actual_current_power = house_power / num_inverters
    # Limit scaling factor to reasonable range (0.5 to 2.0)
    scaling_factor = max(0.5, min(2.0, actual_current_power / typical_current_hour_power))

    for i in range(hours_to_check):
        # Get the hour we're calculating for, wrapping around to 0 if we go past 23
        forecast_hour = (current_hour + i) % 24
        # Get minimum power for this hour and adjust for number of inverters
        typical_hour_power = min_house_power[forecast_hour] / num_inverters

        if i == 0:
            # For current hour, use actual consumption
            hour_consumption = actual_current_power
        else:
            # For future hours, scale the typical consumption by our current ratio
            hour_consumption = typical_hour_power * scaling_factor

        # Convert Wh to kWh for this hour
        total_consumption += hour_consumption / 1000

    return total_consumption, scaling_factor  # Return scaling factor for logging

# Calculate expected consumption and required minimum SOC
estimated_consumption_kW, consumption_scaling = calculate_expected_consumption()
required_min_soc = reserve_factor * (estimated_consumption_kW / battery_capacity) * 100  # Convert to percentage

# Add scaling factor to code for logging
code += f'Scale: {consumption_scaling:.2f}, '
...
if daytime:
    ...
    reason = update_reason(
        ...
        est_consumption_kW=estimated_consumption_kW
    )
...
if battery_soc < full_battery and (start_charging_time <= current_hour < peak_time):
    action = 'import'
    solar = 'export'
    code += 'IMPORT to fill battery before peak'
    reason = update_reason(
        facility_name, buy_price, sell_price, min(discounted_buy_forecast), max(discounted_sell_forecast),
        discounted_buy_forecast.index(min(discounted_buy_forecast)), discounted_sell_forecast.index(max(discounted_sell_forecast)),
...
# Opportunistic buy daytime
if battery_soc < full_battery and (buy_price <= max_day_opportunistic_buy_price):
    action = 'import'
    solar = 'export'
    code += 'Opportunistic Buy, '
    reason = update_reason(
        facility_name, buy_price, sell_price, min(discounted_buy_forecast), max(discounted_sell_forecast),
        discounted_buy_forecast.index(min(discounted_buy_forecast)), discounted_sell_forecast.index(max(discounted_sell_forecast)),
        effective_house_power, sunrise_plus_active, sunset_minus_active,
